...
export function retrieveContext(query, selectedProduct) {
  ...
  if (
    ...
    q.includes('h3') || q.includes('k27m') || q.includes('modeyso') ||
    q.includes('ziihera') || q.includes('testing') || q.includes('biomarker') ||
    q.includes('pediatric') || q.includes('her2') || q.includes('social')
  ) {
    context.push(
  ...
  if (
    ...
    q.includes('alignment') || q.includes('merchant') || q.includes('venkatramani') ||
    q.includes('community') || q.includes('access') || q.includes('payer') ||
    q.includes('social') || q.includes('share of voice') || q.includes('divergence')
  ) {
    ...
    context.push(
      '## Strategic Insights\n' +
      toShow.map(i =>
        `- **${i.title}** (${i.priority} priority, confidence ${Math.round(i.confidenceScore * 100)}%, ` +
        `velocity ${i.socialSignalVelocity || 'N/A'}): ${i.summary}`
      ).join('\n')
    );
  }
  ...
  if (
    ...
    q.includes('merchant') || q.includes('venkatramani') || q.includes('janjigian') ||
    q.includes('wen') || q.includes('monje') || q.includes('shitara') || q.includes('mueller')
  ) {
  ...
  if (
    ...
    q.includes('esmo') || q.includes('sno') || q.includes('abstract') || q.includes('poster')
  ) {
  ...
  if (
    ...
    q.includes('diffuse midline') || q.includes('ziihera') || q.includes('zanidatamab') ||
    q.includes('btc') || q.includes('gec') || q.includes('biliary') || q.includes('gastro') ||
    q.includes('her2') || q.includes('product portfolio')
  ) {
    context.push(
      '## Product Portfolio\n' +
